...
class BoundGasProfile(ccl.halos.profiles.profile_base.HaloProfile): 
    ...
    def _real(self, cosmo, r, M, scale_a=1, Gamma=1.1, call_interp=True): 
        ...
        if call_interp is False:
            print(call_interp)
            vB_prefix = self._Vb_prefix(1/(Gamma-1), r_s)
        else:
...
class CombinedGasProfile(ccl.halos.profiles.profile_base.HaloProfile): 
    ...
    def __init__(self, cosmo, mass_def, concentration, fourier_analytic = True, gammaRange = (3, 20), ngamma=64, qrange=(1e-4, 1e2), nq=64, limInt=(1E-3, 5E3)): 
    ...
    def _fourier_analytic(self, k, M, scale_a=1, Gamma=1.1, f_bd = 1):
        f_ej = 1 - f_bd
...
class CombinedStellarGasProfile(ccl.halos.profiles.profile_base.HaloProfile): 
    ...
    def __init__(self, cosmo, mass_def, concentration, fourier_analytic = True, gammaRange = (3, 20), ngamma=64, qrange=(1e-4, 1e2), nq=64, limInt=(1E-3, 5E3), beta=0.6, M_c = 10**(13.5), M_star = 10**(12.5), A_star = 0.03, sigma_star = 1.2):
